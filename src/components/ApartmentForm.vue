	...
	<div class="container mt-5">
		<form @submit.prevent="handleFormSubmit" class="row g-3">
			...
			<div class="col-lg-6 col-sm-12">
				...
				<input type="text"
					:class="[{ 'is-invalid': errors.name && !form.name }, 'form-control']"
					id="name" name="name" v-model.trim="form.name">
				<div
					:class="[{ 'd-block': errors.name && !form.name }, 'invalid-feedback']">
					{{ errors.name }}
				</div>
			</div>
			...
			<div class="col-lg-6 col-sm-12">
				...
				<input @input="handleAddressInput" @focusout="getCoordinates"
					type="text" v-model.trim="form.address" list="addresses"
					:class="[{ 'is-invalid': errors.address && !form.address }, 'form-control']"
					id="address" />
				<div
					:class="[{ 'd-block': errors.address && !form.address }, 'invalid-feedback']">
					{{ errors.address }}
				</div>
				<datalist id="addresses">
					<option v-for=" address  in  suggestedAddresses " :key="address"
			...
			<div class="col-12">
				...
				<textarea
					:class="[{ 'is-invalid': errors.description && !form.description }, 'form-control']"
					id="description" name="description" rows="7"
					v-model.trim="form.description"></textarea>
				<div
					:class="[{ 'd-block': errors.description && !form.description }, 'invalid-feedback']">
					{{ errors.description }}
				</div>
			</div>
			...
			<div class="col-12">
				...
				<div
					:class="[{ 'd-block': errors.image && !form.image }, 'invalid-feedback']">
					{{ errors.image }}
				</div>
			</div>
			...
			<div class="col-lg-3 col-sm-6">
				...
				<div
					:class="[{ 'd-block': errors.bedrooms && !form.bedrooms }, 'invalid-feedback']">
					{{ errors.bedrooms }}
				</div>
			</div>
			...
			<div class="col-lg-3 col-sm-6">
				...
				<input type="number" name="rooms" id="rooms"
					:class="[{ 'is-invalid': errors.rooms && !form.rooms }, 'form-control']"
					v-model.trim="form.rooms">
				<div
					:class="[{ 'd-block': errors.rooms && !form.rooms }, 'invalid-feedback']">
					{{ errors.rooms }}
				</div>
			</div>
			...
			<div class="col-lg-3 col-sm-6">
				...
				<div
					:class="[{ 'd-block': errors.bathrooms && !form.bathrooms }, 'invalid-feedback']">
					{{ errors.bathrooms }}
				</div>
			</div>
			...
			<div class="col-lg-3 col-sm-6">
				...
				<div
					:class="[{ 'd-block': errors.square_meters && !form.square_meters }, 'invalid-feedback']">
			...
			<div class="col-12">
				<div class="form-check form-switch">
					<label v-if="!form.visible" class="form-check-label"
						for="visible">Rendi visibile</label>
					<label v-else class="form-check-label" for="visible">Rendi
						invisibile</label>
					<input class="form-check-input" type="checkbox" id="visible"
						name="visible" v-model.trim="form.visible">
				</div>
				<div v-if="!apartment.slug"
					class="col-12 mt-4 text-sm-center text-md-start">
					<button type="submit" class="btn btn-primary">Crea
						appartamento</button>
					<button v-if="isBackValid" type="button"
						class="btn btn-secondary mx-2" @click="$router.back">Torna
						indietro</button>
					<RouterLink v-else :to="{ name: 'home' }"
						class="btn btn-secondary mx-2">Torna indietro</RouterLink>
				</div>
